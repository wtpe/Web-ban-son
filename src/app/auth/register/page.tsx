...
export default function  Register (){
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setLoding(true);
    if (!formData.email) {
      setError({ ...error, email: "Bắt buộc nhập email" })
      return;
    }
    if (!formData.password) {
      setError({ ...error, password: "Bắt buộc nhập mật khẩu" })
      return;
    }
    if (!formData.name ) {
      setError({ ...error, name: "Bắt buộc nhập tên" })
      return;
    }
...

              {
                                    loading ? <button type="button" className="w-full flex items-center justify-center text-white bg-orange-600 hover:bg-orange-700 focus:ring-4 focus:outline-none focus:ring-orange-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                                        <TailSpin
                                            ...
                                            visible={true}
                                        />
                                        </button> : <button type="submit" className="w-full text-white bg-orange-600 hover:bg-orange-700 focus:ring-4 focus:outline-none focus:ring-orange-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Đăng ký</button>
                                }

              
              <p className="text-sm  text-gray-500 ">
                Bạn đã có tài khoản ?  <Link href="/auth/login" className="font-medium text-orange-600 hover:underline ">Đăng nhập</Link>
              </p>
            </form>
          </div>
        </div>
