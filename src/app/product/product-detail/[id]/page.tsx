                    ...
                    isLoading ?
                        ...
                        <div className='lg:w-5/6 w-full h-full  bg-gray-100 rounded-xl  flex flex-col lg:flex-row items-center justify-center shadow-2xl  '>
                            <div className='lg:w-4/12 w-full h-60  lg:h-full  rounded-xl z-10 relative'>
                                <Image src={prodData?.productImage || '' } alt='no image' fill className='rounded-xl' />
                            </div>
                            <div className='lg:w-8/12 w-full px-3   rounded flex flex-col lg:px-5 py-2'>
                                ...
                                <h4 className='text-xl font-semibold text-black py-2'>Giá : {`${prodData?.productPrice.toLocaleString("vi-VN", { style: "currency", currency: "VND" })}`}
                                </h4>
                                <h4 className='text-xl font-semibold text-black py-2'>Trọng lượng : {`${prodData?.productWeight} kg`}
                                </h4>
                                <h4 className='text-xl font-semibold text-black py-2'>Định mức : {`${prodData?.productNorm}`}
                                </h4>
                                <p className=' py-2   lg:h-40 w-full'>
                                    {prodData?.productDescription}
                                </p>
                                <div className='w-full py-2 lg:flex-row flex-col flex '>
                                    <button onClick={AddToCart} className='btn m-2 lg:w-52 h-10 btn-outline btn-success flex items-center justify-center'> <BiCartAdd className='text-3xl mx-2' /> Thêm vào giỏ hàng</button>
                                    <button onClick={AddToBookmark} className='btn m-2  lg:w-52 h-10 btn-outline btn-success flex items-center justify-center'> <RiBookMarkFill className='text-3xl mx-2' />Ưa thích</button>
                                </div>
