...
export default function Page({ params, searchParams }: { params: pageParam, searchParams: any }) {
    ...
    return (
        <div className='w-full h-screen dark:text-black bg-gray-50 py-4 px-2 '>
            ...
            <div className='w-full   flex items-start justify-center flex-wrap overflow-auto'>
                {
                    isLoading ? <Loading /> : <>
                         {
                ...
                {
                    isLoading === false && thisProduct ===  undefined || thisProduct?.length <  1 && <p className='text-2xl my-4 text-center font-semibold text-red-400'>Không tìm thấy sản phẩm trong danh mục</p>
                }
            </div>
            <ToastContainer />
        </div>
